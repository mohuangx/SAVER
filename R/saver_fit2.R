...
saver.fit <- function(x, x.est, do.fast, sf, scale.sf, pred.genes, pred.cells,
  ...
  if (do.fast & !null.model) {
    ...
    n2 <- min(max(100, nworkers*2), npred)
    ind2 <- ind[(n1+1):n2]
    ...
    n3 <- min(max(ceiling(n2/mean(info$maxcor[ind[1:n2]] > cutoff)),
                  nworkers + n2), npred)
    ind3 <- ind[(n2+1):n3]
    ...
    out <- calc.estimate(x[ind3, , drop = FALSE], x.est, cutoff, coefs = NULL, 
                         ...
                         null.model, nworkers, calc.maxcor = TRUE, sind = n2+1)
    est[ind3, ] <- out$est
    se[ind3, ] <- out$se
    for (j in 1:6) {
    ...
    out <- calc.estimate(x[ind4, , drop = FALSE], x.est, cutoff, coefs, sf, 
                         ...
                         null.model, nworkers, calc.maxcor = TRUE, sind = n3+1)
    ...
    if (n4 == ngenes) {
      ...
      out <- calc.estimate(x[ind5, , drop = FALSE], x.est, cutoff = 0, 
                           ...
                           calc.maxcor = FALSE, sind = npred+1)
      est[ind5, ] <- out$est
      se[ind5, ] <- out$se
      for (j in 1:6) {
        info[[j+1]][ind5] <- out[[j+2]]
